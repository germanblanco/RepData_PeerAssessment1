...
The maximum value of average steps across all days is `r sprintf("%.2f", max.steps)`, which happens at `r sprintf("%02d:%02d", max.steps.interval%/%100, max.steps.interval%% 100)`.
...
The result of the new calculation returns a value of `r sprintf("%.2f", total.mean.per.day.filled.in)` mean steps taken per day, and `r sprintf("%.2f", total.median.per.day.filled.in)` as the value of the median per day. As expected, the mean is the same since new values have the same value as the mean. The median has changed and it is no longer an integer value, since the mean values that have been added were not rounded to integers. The difference doesn't seem noticeable if we look at the total number of steps either. After looking a bit more into the missing values, it seems that they all correspond to complete missing days (`r unique(activity[is.na(activity$steps),]$date)`). It can be checked with the following code, which returns the number of non NA values for the dates with any NA value (should return 0):
